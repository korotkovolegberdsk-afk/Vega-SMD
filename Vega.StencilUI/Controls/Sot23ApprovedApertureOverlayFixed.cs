...
internal static class Sot23ApprovedApertureOverlay
{
    ...
    private static void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        preview.Dispatcher.BeginInvoke(DispatcherPriority.ContextIdle, new Action(() =>
        {
            ...
            if (layer is null || source is null)
                return;
            foreach (var existing in layer.GetAdorners(preview) ?? Array.Empty<Adorner>())
    ...
    {
        var path = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "..", "Images", "рисунок аперту.png"));
        if (!File.Exists(path))
            return null;
