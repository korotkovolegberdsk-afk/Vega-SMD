...
public sealed class ChipLibraryTests
{
 ...
 [Fact] public void NoAliasBelongsToMultiplePackages(){var s=S();var packages=s.GetAll();var owners=packages.SelectMany(p=>s.GetAliases(p.Id).Select(a=>new{Alias=a.Alias,Package=p.PackageName})).GroupBy(x=>x.Alias,StringComparer.OrdinalIgnoreCase).Where(g=>g.Select(x=>x.Package).Distinct(StringComparer.OrdinalIgnoreCase).Count()>1).ToList();Assert.Empty(owners);}
}
